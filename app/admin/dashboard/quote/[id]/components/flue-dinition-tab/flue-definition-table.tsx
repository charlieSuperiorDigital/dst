...
const FlueTable = ({ quoteId }: Props) => {
  ...
  const fetchData = async () => {
    try {
      const response: PartWithFlues[] = await apiRequest({
        url: `/api/definition/flue/${quoteId}`,
        method: "get",
      });
  ...
  const allBays = Array.from(
    new Set(
      partsWithBays.flatMap(
        (part) => part.flues?.map((bay) => bay.flueName) || []
      )
    )
  );
  if (loading) {
    ...
    for (let rowOffset = 0; rowOffset < copiedCells.length; rowOffset++) {
      for (let colOffset = 0; colOffset < copiedCells[0].length; colOffset++) {
        ...
        if (row < newPartsWithBays.length && col < allBays.length) {
          ...
          const bay = part.flues.find((b) => b.flueName === allBays[col]);
          if (bay) {
            const newQuantity =
  ...
  const handleArrowInEdit = (
    ...
    const currentCellContent =
      partsWithBays[editingCell.row].flues[editingCell.col].quantity.toString();
  ...
  const handleDrop = (
    ...
    if ("selectionRange" in dragSource && dragSource.selectionRange) {
      ...
      for (let row = range.startRow; row <= range.endRow; row++) {
        for (let col = range.startCol; col <= range.endCol; col++) {
          ...
          if (
            ...
            const sourceBay = sourcePart.flues[col];
            const targetBay = targetPart.flues[newCol];

            if (sourceBay && targetBay) {
              targetBay.quantity = sourceBay.quantity;
              updates.push({
                partId: targetPart.part.id,
                flueId: targetBay.flueId,
      ...
      if (
        ...
        const sourceBay = sourcePart.flues[sourceCol];
        const targetBay = targetPart.flues[targetCol];
  ...
  const autoSizeColumn = (colIndex: number) => {
    ...
    partsWithBays.forEach((part) => {
      const cellContent = part.flues[colIndex].quantity.toString();
  ...
  const updateMultipleQuantities = async (
    updates: { partId: string; flueId: string; quantity: number }[]
  ) => {
    try {
      const results = await Promise.allSettled(
        updates.map((update) =>
          apiRequest({
            url: `/api/part/flue/updatePart`,
            method: "put",
            data: update,
          })
        )
      ...
      results.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          toast({
            title: "Error",
            description: `Failed to update quantity for part ${updates[index].partId} and bay ${updates[index].flueId}.`,
            variant: "destructive",
          });
        }
      });
          ...
          prevPartsWithBays.map((partWithBays) => {
            const updatedBays = partWithBays.flues.map((bay) => {
              const update = successfulUpdates.find(
                (u) =>
                  u.partId === partWithBays.part.id && u.flueId === bay.flueId
              );
              return update ? { ...bay, quantity: update.quantity } : bay;
            });
            return { ...partWithBays, flues: updatedBays };
          })
        );
    ...
    try {
      ...
      setPartsWithBays((prevPartsWithBays) =>
        prevPartsWithBays.map((partWithBays) => {
          ...
          return { ...partWithBays, flues: updatedBays }; // Return the updated part
        })
      );
      setFluesDefinitionContext?.((prevPartsWithBays) =>
        prevPartsWithBays.map((partWithBays) => {
  ...
  const calculateTotalQuantity = (partWithBays: PartWithFlues): number => {
    return (partWithBays.flues || []).reduce(
  ...
  const filteredPartsWithBays = partsWithBays.filter((partWithBays) => {
    const matchesSearch = partWithBays.flues.some((bay) =>
      bay.flueName.toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            {filteredPartsWithBays.map((partWithBays, rowIndex) => {
              ...
              return (
                <tr key={partWithBays.part.id}>
                  ...
                  {filteredBays.map((bayName, colIndex) => {
                    const bay = partWithBays.flues.find(
                      (b) => b.flueName === bayName
                    );
                        ...
                        {isEditingCell(rowIndex, colIndex) ? (
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              const bay = part.flues.find(
                                (b) => b.flueName === filteredBays[colIndex]
                              );
