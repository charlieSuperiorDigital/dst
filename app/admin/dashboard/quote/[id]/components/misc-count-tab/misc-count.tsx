...
const MiscTable = ({ quoteId }: Props) => {
  ...
  const updateMultipleQuantities = async (
    updates: { partid: string; rowId: string; quantity: number }[]
  ) => {
    try {
      console.log(updates);
      ...
      results.forEach((result, index) => {
        if (result.status === "fulfilled") {
          ...
          toast({
            title: "Error",
            description: `Failed to update quantity for part ${updates[index].partid} and bay ${updates[index].rowId}.`,
            variant: "destructive",
          });
        }
      });
          ...
          prevPartsWithBays.map((partWithBays) => {
            const updatedBays = partWithBays.rows.map((bay) => {
              const update = successfulUpdates.find(
                (u) =>
                  u.partid === partWithBays.part.id && u.partid === bay.rowId
              );
  ...
  const handleBlur = () => {
    if (debounceTimeout.current) {
      ...
      const update = {
        partid: bayWithRows[editingCell.row].part.id,
              ...
              return (
                <tr key={partWithBays.part.id}>
                  <td
                    ...
                    title={`${partWithBays.part.description}`} // Tooltip for full text
